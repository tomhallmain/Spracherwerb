...
# Type alias for optional host-project string similarity (e.g. Utils.is_similar_strings).
...
class RedundancyPolicy(Protocol):
    ...
    def on_chunk(self, chunk) -> RedundancyVerdict:
        """*chunk* is a :class:`extensions.llm.StreamChunk`."""
        ...
...
def _default_similarity(a: str, b: str) -> bool:
    try:
        ...
        return Utils.is_similar_strings(a, b)
    except Exception:
...
class DefaultRedundancyPolicy:
  ...
  Parameters mirror :class:`tts.chunker.Chunker` tier semantics where noted.
    """
