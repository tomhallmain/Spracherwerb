...
``status`` (the default) prints no secrets -- it reports presence, location
and freshness so you can answer "where is it, has the migration finished, and
is my off-drive backup current?" without opening anything that contains a key.
It does not write key material itself, but importing the app's ``utils``
package builds the AppInfoCache singleton, which decrypts the real cache; on a
machine that has not migrated yet, that first decrypt completes the migration
...
change. Routine backups are taken whenever the key store is written (see
AUTO_BACKUP_KEY_STORE in utils/encryptor.py).
...
def _identifiers(include_legacy: bool) -> list:
    ...
    migrated. AppInfo.LEGACY_APP_IDENTIFIERS is empty, so ``--include-legacy``
    reports nothing extra -- it is kept so a future rename only has to add the
    old name there.
    """
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    print(f"{CACHE_DIR_ENV_VAR}: {os.environ.get(CACHE_DIR_ENV_VAR) or '(unset)'}")
