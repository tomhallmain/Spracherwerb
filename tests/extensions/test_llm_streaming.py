...
class TestLLMStreamingFlags:
    ...
    def test_from_config_reads_flags(self):
        class _Cfg:
            llm_model_name = "test-model"
            llm_use_streaming = True
            llm_stream_redundancy = True
            llm_thinking_budget_chars = 1234
            llm_track_prompts_and_responses = True

        llm = LLM.from_config(_Cfg(), state_key="unit")
        assert llm.model_name == "test-model"
        assert llm.use_streaming is True
        assert llm.use_redundancy_elimination is True
        assert llm.thinking_budget_chars == 1234
        assert llm.track_prompts_and_responses is True
