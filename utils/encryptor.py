...
#: Redirects the whole key store, so a test or a packaged install can point it
#: somewhere else. The same variable utils/cache_paths.py resolves for the rest
#: of the app; named here so this module needs nothing from the application.
CACHE_DIR_ENV_VAR = "SPRACHERWERB_CACHE_DIR"

# {(service_name, app_identifier): dict}. Key material is read once per
# process: load_private_key runs on every encrypt and decrypt, so without this
# the keychain and disk are hit again for every AppInfoCache load and store.
...
def key_store_dir(service_name: str) -> str:
    ...
    platform is user_data_dir()'s business. CACHE_DIR_ENV_VAR still wins.
    """
...
KEY_BACKUP_DIR_ENV_VAR = "SPRACHERWERB_KEY_BACKUP_DIR"
...
class PasswordManager:
    ...
    ciphertext does. Chunked into keychain items each one costs several more
    macOS prompts.
    """
