...
Opt out: set ``SPRACHERWERB_SKIP_VLC_PLUGIN_CACHE_FIX=1`` (or legacy
``MUSE_SKIP_VLC_PLUGIN_CACHE_FIX=1``), or set ``auto_fix_vlc_plugin_cache`` to
...
def _config_allows_fix() -> bool:
    for env_name in ("SPRACHERWERB_SKIP_VLC_PLUGIN_CACHE_FIX", "MUSE_SKIP_VLC_PLUGIN_CACHE_FIX"):
        skip = os.environ.get(env_name, "").strip().lower()
...
def ensure_vlc_plugin_cache_if_stale() -> None:
    ...
    with _ensure_lock:
        ...
        try:
            ...
            if system == "Windows":
                logger.info(
                    "VLC plugin cache appears stale (%s). Restart VLC or run vlc-cache-gen manually.",
                    plugins_dir,
                )
                return
